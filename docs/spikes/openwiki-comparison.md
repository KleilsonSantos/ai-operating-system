...
1. **AGENTS.md as pointer, not dump** — OpenWiki’s “don’t stuff the repo into AGENTS.md; point at an index” matches AIOS _policies > long prompts_. We can harden `AGENTS.md` to point at FOUNDATION + PKB index without adopting OpenWiki.
...
3. **OKF-style front matter** — Optional schema for generated _context pages_ (if we ever emit them), kept **below** FOUNDATION/ADRs in authority.
