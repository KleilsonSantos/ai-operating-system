...
export function createAiosMcpServer(): McpServer {
  ...
  server.registerTool(
    'aios_contract_version',
    {
      ...
      description: 'Returns the stable Pipeline contractVersion used by @aios/pipeline / CLI.',
    },
    async () => ({
      content: [
        {
  ...
  server.registerTool(
    'aios_list_workspaces',
    {
      ...
      inputSchema: {
        ...
        workspacesPath: z.string().optional().describe('Override path to aios.workspaces.json'),
      },
    },
    async ({ homePath, workspacesPath }) => {
      const cwd = resolve(homePath || process.env.AIOS_HOME || process.cwd());
  ...
  server.registerTool(
    'aios_run_across_workspaces',
    {
      ...
      inputSchema: {
        ...
        workspaceIds: z.array(z.string()).optional().describe('Subset of ids; default = all'),
        homePath: z.string().optional(),
    ...
    async ({ input, workspaceIds, homePath, scope }) => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text', text: `aios_run_across_workspaces failed: ${message}` }],
          isError: true,
        };
      }
    }
  ...
  server.registerTool(
    'aios_build_knowledge',
    {
      ...
      inputSchema: {
        repoPath: z.string().optional().describe('Repo root (default: AIOS_REPO / cwd)'),
        workspaceId: z.string().optional().describe('Workspace id (resolves repoPath)'),
        full: z
          .boolean()
          .optional()
    ...
    async ({ repoPath, workspaceId, full }) => {
      ...
      const payload = full ? graph : { ...summarizeKnowledge(graph), repoPath: graph.repoPath };
      return {
  ...
  server.registerTool(
    'aios_memory_remember',
    {
      ...
      inputSchema: {
        workspaceId: z.string().describe('Workspace id (e.g. aios)'),
  ...
  server.registerTool(
    'aios_load_policies',
    {
      ...
      inputSchema: {
        repoPath: z.string().optional().describe('Repo root (default: process.cwd() / AIOS_REPO)'),
        workspaceId: z
          .string()
          .optional()
          .describe('Workspace id from registry (resolves repoPath)'),
        policiesPath: z.string().optional().describe('Optional path to policies JSON'),
      },
    },
  ...
  server.registerTool(
    ...
    async ({ homePath, provider }) => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text', text: `aios_governance_status failed: ${message}` }],
          isError: true,
        };
      }
    }
  ...
  server.registerTool(
    ...
    async ({ homePath, repoPath, includeDocumentation }) => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text', text: `aios_governance_audit failed: ${message}` }],
          isError: true,
        };
      }
    }
  ...
  server.registerTool(
    'aios_governance_record',
    {
      ...
      description: 'Append a light decision to .aios/governance/decisions.jsonl (#80).',
      inputSchema: {
    ...
    async ({ summary, kind, verdict, policyIds, homePath }) => {
      try {
        ...
        const message = err instanceof Error ? err.message : String(err);
        return {
          content: [{ type: 'text', text: `aios_governance_record failed: ${message}` }],
          isError: true,
        };
      }
    }
  ...
  server.registerTool(
    'aios_provider_health',
    {
      ...
      inputSchema: {
        ...
        baseUrl: z.string().optional().describe('Override base URL (Ollama or OpenAI-compatible)'),
      },
    },
      ...
      try {
        ...
        return {
          content: [
            {
              type: 'text',
              text: JSON.stringify({ provider: p.id, count: models.length, models }, null, 2),
            },
          ],
        };
      } catch (err) {
  ...
  server.registerTool(
    'aios_run_pipeline',
    {
      ...
      inputSchema: {
        input: z.string().describe('Short user intent, e.g. "Analise meu projeto."'),
        repoPath: z.string().optional().describe('Target repository root (wins over workspaceId)'),
        workspaceId: z.string().optional().describe('Id from workspaces/aios.workspaces.json'),
        scope: z
          .string()
          .optional()
