...
describe('loadPolicies', () => {
  ...
  it('descobre policies/ subindo diretórios (monorepo)', () => {
    ...
    writeFileSync(
      ...
      JSON.stringify([{ id: 'from-root', description: 'via walk-up', severity: 'should' }])
    );
  ...
  it('respeita configPath explícito sem merge', () => {
    ...
    const only: PolicyRule[] = [{ id: 'custom', description: 'Only custom', severity: 'may' }];
  ...
  it('rejeita severity inválida', () => {
    ...
    writeFileSync(file, JSON.stringify([{ id: 'x', description: 'y', severity: 'nope' }]));
    expect(() => loadPolicies({ configPath: file, mergeDefaults: false })).toThrow(/severity/);
  });
});
...
describe('mergePolicies', () => {
  it('overlay substitui por id', () => {
    ...
    const overlay: PolicyRule[] = [{ id: 'a', description: 'new', severity: 'may' }];
