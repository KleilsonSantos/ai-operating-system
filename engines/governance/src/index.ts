...
import type { AttentionItem, GovernanceAudit, GovernanceDecision } from '@aios/shared';
...
export function isKnownDecisionKind(kind: string): boolean {
  return (KNOWN_DECISION_KINDS as readonly string[]).includes(normalizeKind(kind));
}
...
export function recordDecision(
  ...
  const policyIds = input.policyIds?.map((id) => id.trim()).filter(Boolean);
...
export function listDecisions(options: GovernanceOptions & { limit?: number } = {}): {
  path: string;
...
export function auditGovernance(options: AuditGovernanceOptions = {}): GovernanceAudit {
  ...
  const missingCoreMustIds = PLATFORM_CORE_MUST_IDS.filter((id) => !applied.mustIds.includes(id));
  ...
  return {
    ...
    policies: {
      ...
      missingCoreMustIds: missingCoreMustIds.length > 0 ? [...missingCoreMustIds] : undefined,
    },
    decisions: {
      path: listed.path,
      ...
      unknownPolicyIds: unknownPolicyIds.size > 0 ? [...unknownPolicyIds] : undefined,
    },
    documentation,
    findings,
    ok,
