...
`;
    expect(parsePkbIndexPaths(yaml)).toEqual(['by-domain/a/x.v1.md', 'by-domain/b/y.v1.md']);
  });
});
...
describe('searchPkb', () => {
  function seedRepo(): string {
    ...
    writeFileSync(
      join(root, 'docs', 'prompts', 'by-domain', 'documentation', 'readme-audit.v1.md'),
      `---
...
`
    ...
    writeFileSync(
      join(root, 'docs', 'prompts', 'by-domain', 'knowledge', 'kb-evolution.v1.md'),
      `---
...
describe('auditDocumentation', () => {
  it('reporta gaps num repo vazio', () => {
    ...
    expect(audit.findings.some((f) => f.id.startsWith('pkb-missing-'))).toBe(true);
  });
  ...
  it('ok parcial com docs mínimas', () => {
    ...
    writeFileSync(join(root, 'docs', 'adr', '0001-standalone-platform.md'), '# adr');
    writeFileSync(join(root, 'docs', 'adr', '0002-git-branching-strategy.md'), '# adr');
    writeFileSync(join(root, 'docs', 'adr', '0003-pipeline-integration-contract.md'), '# adr');
    ...
    writeFileSync(join(root, 'docs', 'prompts', 'by-domain', 'demo', 'asset.v1.md'), '# asset');
    writeFileSync(join(root, 'docs', 'prompts', 'by-domain', 'demo', 'orphan.v1.md'), '# orphan');
  ...
  it('warns when index path is missing on disk', () => {
    ...
    const audit = auditDocumentation({ repoPath: root });
    expect(audit.ok).toBe(true);
    expect(audit.findings.some((f) => f.id.includes('pkb-index-missing'))).toBe(true);
  });
});
