...
import type { OperationalGitSnapshot, OperationalState } from '@aios/shared';
...
export type GetOperationalStateOptions = {
  ...
  providerHealth?: Awaited<ReturnType<typeof getGovernanceStatus>>['provider'];
};
...
export function probeGit(cwd: string): OperationalGitSnapshot {
  try {
    ...
    const inside = execFileSync('git', ['rev-parse', '--is-inside-work-tree'], opts).trim();
    if (inside !== 'true') {
...
function buildSummary(parts: {
  ...
  const bits: string[] = [];
  bits.push(parts.workspaceCount === 0 ? 'sem workspaces' : `${parts.workspaceCount} workspace(s)`);
