...
- 📦 `@aios-platform/agent-template` — extract scaffold template from create-agent; `resolveTemplateDir()`; destination filenames (no `.tmpl`; create-vite style) (#532)
...
- 🔔 DecisionRecord `failed`/`denied` → Console Attention via bounded run-store skim (#524)
...
- 📋 Delivery automation map — event → CI → agent next step (`docs/guides/delivery-automation.md`) (#448)
